...
for (eachbase in base_list) {
  ...
  base_raw <- read_dta(paste0(data,"raw/base_",eachbase,"_alum_dir_2016.dta"))
  ...
  if (eachbase == "56anio") {
    ...
    base <- base %>%
      mutate(repitio = case_when(repitio_prim == 1 ~ 1,
                                 repitio_sec == 1  ~ 1,
                                 repitio_prim == 0 & repitio_sec == 0 ~ 0))
  ...
  write.csv(base,paste0(data,"analytical/base_",eachbase,"_2016.csv"))
